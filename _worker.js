...
export default {
  async fetch(request, env, ctx) {
    ...
    const 管理员密码 = env.ADMIN || env.admin || env.PASSWORD || env.password || env.TOKEN || env.KEY || env.UUID || env.uuid;
...
function MD5(str) {
  return crypto.subtle.digest('MD5', new TextEncoder().encode(str)).then(h => 
...
async function 请求优选API(urls, 默认端口 = '443', 超时 = 3000) {
  ...
  for (const r of all) {
    if (r.status === 'fulfilled') res.push(...r.value);
  }
